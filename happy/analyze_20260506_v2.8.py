#!/usr/bin/env python3
"""
第1933期彩票数据分析 - ML-Integrated V2.8
分层数据架构: 全量历史(1908期基准) + 近期窗口(17期)
"""
import random
...
# 2. 本期数据（第1933期 - 用户提供）
# =============================================================
NEW_DRAW = [10, 12, 23, 27, 36, 38, 41, 45, 47, 50, 53, 58, 61, 62, 65, 67, 69, 72, 75, 76]

# =============================================================
# V2.7 第1933期预测（用于验证）
# =============================================================
V2_7_PRED_1933 = [1, 6, 7, 15, 19, 22, 31, 32, 34, 35, 48, 52, 54, 56, 59, 63, 64, 72, 73, 78]
...
ALL_HISTORY_FULL = [
    ...
    [10, 12, 23, 27, 36, 38, 41, 45, 47, 50, 53, 58, 61, 62, 65, 67, 69, 72, 75, 76], # 1933 (NEW)
]
...
repeat_nums_1932 = set(NEW_DRAW) & set(ALL_HISTORY_FULL[-2])
rc = len(repeat_nums_1932)
...
# ---- V2.7 验证 ----
hit_v27 = set(V2_7_PRED_1933) & set(NEW_DRAW)
...
for n in range(1, 81):
    ...
    # 1933: 1-20(2严重不足), 21-40(4不足), 41-60(6略多), 61-80(8严重偏多)
    if 1 <= n <= 20 and ic <= 2: ivs *= 2.5  # 极强反弹
    if 21 <= n <= 40 and ic <= 4: ivs *= 1.5
    if 41 <= n <= 60 and ic >= 6: ivs *= 0.4
    if 61 <= n <= 80 and ic >= 8: ivs *= 0.15  # 极强回落
...
for n in range(1, 81):
    ...
    ic = current_intervals[ni]
    ir = max(0, (5-ic)/5)
    if ni == "1-20" and ic <= 2: ir *= 3.0
    if ni == "21-40" and ic <= 4: ir *= 1.6
    if ni == "41-60" and ic >= 6: ir *= 0.3
    if ni == "61-80" and ic >= 8: ir *= 0.1
...
for n in range(1, 81):
    ...
    if ni == "1-20" and ic <= 2: sc += 0.18  # 极强反弹
    if ni == "21-40" and ic <= 4: sc += 0.08
    if ni == "41-60" and ic >= 6: sc -= 0.06
    if ni == "61-80" and ic >= 8: sc -= 0.15  # 极强回落
    if n in NEW_DRAW: sc += 0.05
    rule_s[n] = sc
...
# =============================================================
print("=" * 78)
print("  📅 第1933期彩票数据完整分析报告")
print("  ML-Integrated V2.8 | 全量(1908期)+近期(17期)")
...
📊 基本统计:
  ...
  重复(对1932): {rc}/20 = {rc/20*100:.0f}%
""")

# --- V2.7 验证 ---
print(f"🔍 V2.7 第1933期预测验证")
print(f"  V2.7 预测(1933): {', '.join(f'{n:02d}' for n in V2_7_PRED_1933)}")
print(f"  实际1933:       {', '.join(f'{n:02d}' for n in NEW_DRAW)}")
print(f"  命中: {sorted(hit_v27)} ({len(hit_v27)}/20 = {len(hit_v27)/20*100:.1f}%)")
print()
...
mr = [
    ...
    ("random_forest", len(set(rf_pred)&set(NEW_DRAW)), rf_pred, 0.24),
    ("xgboost", len(set(xg_pred)&set(NEW_DRAW)), xg_pred, 0.22),
    ("omission", len(set(om_pred)&set(NEW_DRAW)), om_pred, 0.23),
]
...
print(f"  {'─'*34}")
print(f"  V2.7集成: {len(hit_v27)}/20 = {len(hit_v27)/20*100:.0f}%")
print()

# --- 权重调整 ---
OW = {"rule_based":0.31, "random_forest":0.24, "xgboost":0.22, "omission":0.23}
...
# 1933期特征:
# 小号: 30%(极低)  |  奇数: 55%(偏高)
# 1-20: 2(极度不足)  |  21-40: 4(不足)
# 41-60: 6(偏多)     |  61-80: 8(极度偏多)
# 重复: 30%(偏高)

# 下期预测: 小号反弹 + 1-20强反弹 + 61-80回落 + 奇数回补
ICAPS = {"1-20":6, "21-40":5, "41-60":5, "61-80":4}
...
print("═"*60)
print(f"🔗 第1934期预测 (ML-Integrated V2.8)")
print("═"*60)
print(f"""
┌────────────────────────────────────────────────────────────┐
│                                                           │
...
════════════════════════════════════════════════════════════════
  ...
  │ 41-60: 6(偏多)→ 回落 5个        🟡   │
  │ 61-80: 8(极度偏多)→ 强回落 4个  🔴   │
  │ 重复: {rc}/20({rc/20*100:.0f}%) → 预测 25-30%        │
  └──────────────────────────────────────────┘

  💡 本期最大特征:
    • 61-80区间8个号码占40% → 必然强回落⚡
    • 1-20仅2个极度不足 → 强反弹预期⚡
    • 小号30%是近期严重低位 → 均值回归
    • 奇数连续三期偏多(60%→60%→55%) → 继续回落预期
""")

# =============================================================
# 4. 命中率预期系统
# =============================================================
ML_ALL_HIST = [35, 25, 45, 35, 35, 20, 35, 20, 5]
ALL_RATES = [25, 50, 46, 33, 14, 44, 5, 5, 0, 25, 15, 40, 40, 35, 25, 25, 35, 15, 20, 20, 35, 25, 45, 35, 35, 20, 35, 20, 5]
...
cur_hit_pct = len(hit_v27) / 20 * 100
...
═"*60
💾 数据已保存至 happy/ 目录 | 文件: analyze_20260506_v2.8.py
═"*60
""")
