#!/usr/bin/env python3
"""
第1934期彩票数据分析 - ML-Integrated V2.9
分层数据架构: 全量历史(1908期基准) + 近期窗口(18期)
"""
import random
...
# 2. 本期数据（第1934期）
# =============================================================
NEW_DRAW = [2, 5, 6, 9, 17, 19, 20, 26, 29, 31, 32, 33, 37, 40, 44, 53, 58, 62, 71, 78]

V2_8_PRED_1934 = [2, 6, 7, 12, 17, 20, 22, 27, 31, 34, 35, 42, 48, 52, 56, 60, 63, 72, 73, 78]
...
ALL_HISTORY_FULL = [
    ...
    [2, 5, 6, 9, 17, 19, 20, 26, 29, 31, 32, 33, 37, 40, 44, 53, 58, 62, 71, 78],     # 1934 (NEW)
]
...
repeat_nums_1933 = set(NEW_DRAW) & set(ALL_HISTORY_FULL[-2])
rc = len(repeat_nums_1933)
...
hit_v28 = set(V2_8_PRED_1934) & set(NEW_DRAW)
...
for n in range(1, 81):
    ...
    # 1934: 1-20(7极度偏多), 21-40(7极度偏多), 41-60(3不足), 61-80(3不足)
    if 1 <= n <= 20 and ic >= 7: ivs *= 0.2
    if 21 <= n <= 40 and ic >= 7: ivs *= 0.3
    if 41 <= n <= 60 and ic <= 3: ivs *= 2.0
    if 61 <= n <= 80 and ic <= 3: ivs *= 2.0
...
for n in range(1, 81):
    ...
    ic = current_intervals[ni]
    ir = max(0, (5-ic)/5)
    if ni == "1-20" and ic >= 7: ir *= 0.1
    if ni == "21-40" and ic >= 7: ir *= 0.2
    if ni == "41-60" and ic <= 3: ir *= 2.5
    if ni == "61-80" and ic <= 3: ir *= 2.5
...
for n in range(1, 81):
    ...
    if ni == "1-20" and ic >= 7: sc -= 0.15
    if ni == "21-40" and ic >= 7: sc -= 0.12
    if ni == "41-60" and ic <= 3: sc += 0.12
    if ni == "61-80" and ic <= 3: sc += 0.12
...
# =============================================================
print("=" * 78)
print("  📅 第1934期彩票数据完整分析报告")
print("  ML-Integrated V2.9 | 全量(1908期)+近期(18期)")
...
📊 基本统计:
  ...
  重复(对1933): {rc}/20 = {rc/20*100:.0f}%
""")

print(f"🔍 V2.8 第1934期预测验证")
print(f"  V2.8 预测(1934): {', '.join(f'{n:02d}' for n in V2_8_PRED_1934)}")
print(f"  实际1934:       {', '.join(f'{n:02d}' for n in NEW_DRAW)}")
print(f"  命中: {sorted(hit_v28)} ({len(hit_v28)}/20 = {len(hit_v28)/20*100:.1f}%)")
print()
...
mr = [
    ...
    ("random_forest", len(set(rf_pred)&set(NEW_DRAW)), rf_pred, 0.23),
...
print(f"  {'─'*34}")
print(f"  V2.8集成: {len(hit_v28)}/20 = {len(hit_v28)/20*100:.0f}%")
print()

OW = {"rule_based":0.32, "random_forest":0.23, "xgboost":0.22, "omission":0.23}
...
# 1934特征: 小号70%(极度偏多), 奇数50%, 1-20(7),21-40(7),41-60(3),61-80(3)
# 下期: 小号回落 + 41-60和61-80反弹
...
print()
print(f"═"*60)
print(f"🔗 第1935期最终预测 (ML-Integrated V2.9)")
print(f"═"*60)
print(f"""
┌────────────────────────────────────────────────────────────┐
│                                                           │
...
════════════════════════════════════════════════════════════════
  ...
  │ 41-60: 3(不足)    → 反弹 5个    🔴   │
  │ 61-80: 3(不足)    → 反弹 5个    🔴   │
  │ 重复: {rc}/20({rc/20*100:.0f}%) → 预测 15-25%          │
  └──────────────────────────────────────────┘

  💡 本期最大特征:
    • 小号70%为近期极端最高 → 必然强回落 ⚡
    • 1-20和21-40各7个，双区间极度偏多 → 双双回落 ⚡
    • 41-60和61-80各3个，双区间不足 → 双双反弹 ⚡
    • 奇数回归50%正常水平
    • 重复率仅15%很低 → 号码更替加速
""")
...
# 4. 命中率预期系统
# =============================================================
ML_ALL_HIST = [35, 25, 45, 35, 35, 20, 35, 20, 5, 30]
ALL_RATES = [25, 50, 46, 33, 14, 44, 5, 5, 0, 25, 15, 40, 40, 35, 25, 25, 35, 15, 20, 20, 35, 25, 45, 35, 35, 20, 35, 20, 5, 30]
...
cur_hit_pct = len(hit_v28) / 20 * 100
...
═"*60
💾 数据已保存至 happy/ 目录 | 文件: analyze_20260506_v2.9.py
═"*60
""")
