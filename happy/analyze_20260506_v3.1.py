#!/usr/bin/env python3
"""
第1936期彩票数据分析 - ML-Integrated V3.1
分层数据架构: 全量历史(1908期基准) + 近期窗口(20期)
"""
import random
...
NEW_DRAW = [5, 6, 7, 12, 14, 25, 26, 30, 31, 33, 36, 43, 44, 48, 51, 57, 70, 75, 77, 80]

V3_0_PRED_1936 = [3, 4, 7, 8, 15, 22, 24, 28, 34, 35, 42, 48, 52, 56, 60, 63, 69, 73, 74, 77]
...
ALL_HISTORY_FULL = [
    ...
    [5, 6, 7, 12, 14, 25, 26, 30, 31, 33, 36, 43, 44, 48, 51, 57, 70, 75, 77, 80],    # 1936 (NEW)
]
...
repeat_nums_1935 = set(NEW_DRAW) & set(ALL_HISTORY_FULL[-2])
rc = len(repeat_nums_1935)
...
hit_v30 = set(V3_0_PRED_1936) & set(NEW_DRAW)
...
for n in range(1, 81):
    ...
    # 1936: 1-20(5正常), 21-40(6略多), 41-60(5正常), 61-80(4偏少)
    if 21 <= n <= 40 and ic >= 6: ivs *= 0.6
    if 61 <= n <= 80 and ic <= 4: ivs *= 1.3
    # 本期特点: 5开头号码多(5,6,7), 7/12/14/25/26/30/31/33/36 - 小号密集
...
for n in range(1, 81):
    ...
    ic = current_intervals[ni]
    ir = max(0, (5-ic)/5)
    if 21 <= n <= 40 and ic >= 6: ir *= 0.5
    if 61 <= n <= 80 and ic <= 4: ir *= 1.5
...
for n in range(1, 81):
    ...
    if 21 <= n <= 40 and ic >= 6: sc -= 0.06
    if 61 <= n <= 80 and ic <= 4: sc += 0.08
...
# =============================================================
print("=" * 78)
print("  📅 第1936期彩票数据完整分析报告")
print("  ML-Integrated V3.1 | 全量(1908期)+近期(20期)")
...
📊 基本统计:
  ...
  重复(对1935): {rc}/20 = {rc/20*100:.0f}%
""")

prev_rate = 10  # V2.9 hit 10%
cur_rate = len(hit_v30) / 20 * 100
trend_icon = "📈" if cur_rate > prev_rate else ("📉" if cur_rate < prev_rate else "➡️")

print(f"🔍 V3.0 第1936期预测验证 | 上期{prev_rate}%→本期{cur_rate:.0f}% {trend_icon}")
print(f"  V3.0 预测(1936): {', '.join(f'{n:02d}' for n in V3_0_PRED_1936)}")
print(f"  实际1936:       {', '.join(f'{n:02d}' for n in NEW_DRAW)}")
print(f"  命中: {sorted(hit_v30)} ({len(hit_v30)}/20 = {cur_rate:.1f}%)")
print()
...
mr = [
    ("rule_based", len(set(rule_pred)&set(NEW_DRAW)), rule_pred, 0.28),
    ("omission", len(set(om_pred)&set(NEW_DRAW)), om_pred, 0.27),
...
print(f"  {'─'*34}")
print(f"  V3.0集成: {len(hit_v30)}/20 = {cur_rate:.0f}% {trend_icon}")
print()

OW = {"rule_based":0.28, "omission":0.27, "xgboost":0.23, "random_forest":0.22}
...
# 1936特征: 小号55%(正常), 奇数50%, 1-20:5,21-40:6(略多),41-60:5,61-80:4(偏少)
# 下期: 21-40回落，61-80继续反弹，注意热门号码延续
...
print()
print(f"═"*60)
print(f"🔗 第1937期最终预测 (ML-Integrated V3.1)")
print(f"═"*60)
print(f"""
┌────────────────────────────────────────────────────────────┐
│                                                           │
...
# 4. 命中率预期系统
# =============================================================
ML_ALL_HIST = [35, 25, 45, 35, 35, 20, 35, 20, 5, 30, 10, 15]
...
print(f"""
  ...
  最可能区间: 20-30%（当前仍偏低，反弹概率高）

═"*60
💾 数据已保存至 happy/ 目录 | 文件: analyze_20260506_v3.1.py
═"*60
""")
